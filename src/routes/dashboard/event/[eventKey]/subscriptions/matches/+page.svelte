...
{#snippet matchRow(match: M)}
	...
	{@const subs = $subscriptions.filter(
		(s) =>
			s.data.args === `${eventKey}:${match.match.tba.comp_level}:${match.match.tba.match_number}`
	)}
	<tr class="match-row" class:tator={teams.includes(2122)}>
		...
		{#each teams as team, i}
			<td class:text-danger={i < teams.length / 2} class:text-primary={i >= teams.length / 2}>
				{team}
			</td>
		{/each}
		<td>

<script lang="ts">
    ...
    const matches = new WritableArray<Scouting.MatchSchemaType & {
        isError: string;
    }>([]);
    ...
    const uploadFiles = async () => {
        ...
        const parsed = z.array(Scouting.MatchSchema).safeParse(fileData.value.map(d => JSON.parse(d.text)));
        ...
        <Grid data={matches} opts={{
            columnDefs: [
                ...
                },
                {
                    headerName: 'Match',
                    width: 100,
                    cellRenderer: (params: ICellRendererParams<Scouting.MatchSchemaType>) => `${params.data?.compLevel} ${params.data?.match}`,
                },
                {
                    field: 'team',
                    width: 100,
                ...
                },
                {
                    ...
                    headerName: 'Error',
                    width: 500,
                    cellRenderer: (params: ICellRendererParams<Scouting.MatchSchemaType & { isError: string }>) => params?.data?.isError ? `❌ Upload Error: ${params.data.isError}` : '',
                }
            ],
        }} height="80vh" />
    </div>

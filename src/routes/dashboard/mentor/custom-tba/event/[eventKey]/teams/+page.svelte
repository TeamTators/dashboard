<script lang="ts">
    ...
    const save = () => {
        event.setTeams(Array.from(new Set($updateTeams
    ...
    const addTeam = () => {
        ...
        ].filter((t, i, arr) => arr.findIndex(x => x.number === t.number) === i)); // Remove duplicates
        const g = grid.getGrid();
    ...
    const findTeam = async (teamNumber: number) => {
        const t = await get(`/tba/team/frc${teamNumber}`, z.object({
            team: z.object({
...
<div class="container">
    <div class="row mb-3">
        <div class="col">
            <h1>Manage Teams for <span class="text-secondary">{event.tba.name} ({$updateTeams.length})</span></h1>
            <p class="text-muted">
                Add or edit teams for this event. Teams are not synced with TheBlueAlliance and are only available on this instance.
                <br>
                <strong>Note:</strong> Teams must have a valid team number and can be edited to update their TBA data. Any duplicates or invalid teams will be ignored when saving. Any update to the team number will trigger a save.
            </p>
    ...
    <div class="row mb-3">
        <div class="col">
                    ...
                    contextmenu(
                        event.event as MouseEvent,
                        {
                            options: [
                                ...
                                {
                                    action: async () => {
                                        window.open(`https://www.thebluealliance.com/team/${event.data?.number}`, '_blank');
                                    },
            ...
            }} data={updateTeams} style="height: 400px;"/>
        </div>
    </div>

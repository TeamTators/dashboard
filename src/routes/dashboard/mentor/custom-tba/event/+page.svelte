<script lang="ts">
    ...
    let eventState: 'ready' | 'finding' | 'found' | 'not_found' | 'invalid' = $state('ready');
    ...
    const submit = async () => {
        if (dateState === 'valid' && eventName.length >= 3 && eventState === 'not_found' && eventKey && year) {
    ...
    onMount(() => {
        if (eventKey) { // for dev purposes only
            findEvent(eventKey);

...
<div class="container d-flex flex-column align-items-center justify-content-center vh-100">
	<div class="card shadow-sm p-4" style="max-width: 400px; width: 100%;">
		<h5 class="text-center mb-3">
			Do you want to subscribe to <strong class="text-primary">{capitalize(fromSnakeCase(type))}</strong>?
            <br>
		</h5>
        <p class="text-muted">
        ...
        <button type="button" class="btn btn-primary" onclick={async () => {
            const { value: { Preferences } } = await getNotify();
            ...
            if (res.isOk()) {
                notify({
                    ...
                    message: `You have successfully unsubscribed from ${fromSnakeCase(type)}.`,
                    color: 'success'
                });
            } else {
                notify({

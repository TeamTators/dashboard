...
const summary2025 = YearInfo2025.summary({
	'Average Auto Points': {
		...
		Coral: ({ scoring }) => scoring.map((d) => d.auto.cl1 + d.auto.cl2 + d.auto.cl3 + d.auto.cl4),
		Algae: ({ scoring }) => scoring.map((d) => d.auto.brg + d.auto.prc),
	...
	'Average Teleop Points': {
		...
		Algae: ({ scoring }) => scoring.map((d) => d.teleop.brg + d.teleop.prc),
	...
	Stats: {
		'Average Total Points': ({ scoring }) => scoring.map((d) => d.total),

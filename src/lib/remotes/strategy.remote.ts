...
export const fromId = query(
	...
	async ({ id }) => {
		...
		if (partners.isErr() || opponents.isErr())
			return error(503, 'Failed to fetch strategy partners/opponents');
...
export const fromMatch = query(
	...
	async ({ eventKey, matchNumber, compLevel }) => {
		const strategy = await Strategy.Strategy.get(
			{ eventKey, matchNumber, compLevel },
			{ type: 'all' }
		);
		if (strategy.isErr()) return error(503, 'Failed to fetch strategy');

		return Promise.all(
			strategy.value.map(async (s) => {

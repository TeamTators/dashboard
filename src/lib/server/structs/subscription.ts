...
export namespace Subscription {
	...
	export const getSubscriptions = (type: string, args: string) => {
		return attemptAsync(async () => {
			const res = await DB.select()
				.from(WebhookSubscription.table)
				.where(
					and(eq(WebhookSubscription.table.type, type), eq(WebhookSubscription.table.args, args))
				);
	...
	export const init = () => {
		return attempt(() => {
			const service = TBAWebhooks.init(str('LOCAL_TBA_WEBHOOK_REDIS_NAME', true));

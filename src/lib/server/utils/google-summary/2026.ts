...
export const summarize = (eventKey: string) => {
	return attemptAsync(async () => {
		...
		const getTeamStatus = memoize(
			async (team: Team) => {
				return (await team.getStatus()).unwrap()?.qual?.ranking.rank || 'unknown';
			},
		...
		for (const [key, name] of Object.entries(YearInfo2026.actions)) {
			...
			t.column(`Average ${name} Auto Score`, async (t) => {
				return getScores(t).then((scores) =>
					average(scores.traceScore.map((s) => s.auto[key as keyof typeof s.auto] ?? 0))
				);
			});
			t.column(`Average ${name} Teleop Score`, async (t) => {
				return getScores(t).then((scores) =>
					average(scores.traceScore.map((s) => s.teleop[key as keyof typeof s.teleop] ?? 0))
				);
			});
			t.column(`Average ${name} Endgame Score`, async (t) => {
				return getScores(t).then((scores) =>
					average(scores.traceScore.map((s) => s.endgame[key as keyof typeof s.endgame] ?? 0))
				);
			});
			t.column(`StdDev ${name} Auto Score`, async (t) => {
				return getScores(t).then((scores) =>
					standardDeviation(scores.traceScore.map((s) => s.auto[key as keyof typeof s.auto] ?? 0))
				);
			});
			t.column(`StdDev ${name} Teleop Score`, async (t) => {
				return getScores(t).then((scores) =>
					standardDeviation(
						scores.traceScore.map((s) => s.teleop[key as keyof typeof s.teleop] ?? 0)
					)
				);
			});
			t.column(`StdDev ${name} Endgame Score`, async (t) => {
				return getScores(t).then((scores) =>
					standardDeviation(
						scores.traceScore.map((s) => s.endgame[key as keyof typeof s.endgame] ?? 0)
					)
				);
			});
			t.column(`Max ${name} Auto Score`, async (t) => {
				return getScores(t).then((scores) =>
					Math.max(...scores.traceScore.map((s) => s.auto[key as keyof typeof s.auto] ?? 0))
				);
			});
			t.column(`Max ${name} Teleop Score`, async (t) => {
				return getScores(t).then((scores) =>
					Math.max(...scores.traceScore.map((s) => s.teleop[key as keyof typeof s.teleop] ?? 0))
				);
			});
			t.column(`Max ${name} Endgame Score`, async (t) => {
				return getScores(t).then((scores) =>
					Math.max(...scores.traceScore.map((s) => s.endgame[key as keyof typeof s.endgame] ?? 0))
				);
			});
		}

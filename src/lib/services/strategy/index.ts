...
export class Strategy extends WritableBase<StrategyConfig> {
    render(target: HTMLDivElement) {
        ...
        const ondblclick = (event: MouseEvent) => {
            ...
            event.preventDefault();
            oncontextmenu(new PointerEvent('contextmenu', { clientX: event.clientX, clientY: event.clientY }));
        };
        ...
        const isTool = (event: Event) => {
            ...
            if (event.target instanceof HTMLButtonElement || event.target.parentElement instanceof HTMLButtonElement) {
                return true;
            }
        ...
        const ontouchstart = (event: TouchEvent) => {
            ...
            if (now - lastTouchStart < 250) {
                lastTouchStart = 0;
                oncontextmenu(new PointerEvent('contextmenu', { clientX: event.touches[0].clientX, clientY: event.touches[0].clientY }));
                return;
            }
            lastTouchStart = now;
            if (isTool(event)) return;

...
export class Comment extends WritableBase<CommentConfig> {
    render(target: HTMLDivElement) {
        ...
        const renderPos = () => {
            const { position: [posX, posY], size: [width, height] } = this.data;
            ...
            .draggable({
                listeners: {
                    move: (event) => {
                        ...
                        const newPos: Point2D = [position[0] + event.dx / target.clientWidth, position[1] + event.dy / target.clientHeight];
            ...
            .resizable({
                listeners: {
                    move: (event) => {
                        ...
                        const newSize: Point2D = [size[0] + event.deltaRect.width, size[1] + event.deltaRect.height];

			...
			if (this.targetPos && this.currentPos) {
				...
				if (distance > stopThreshold) {
					...
					const decelerationDistance =
						(this.currentVelocity * this.currentVelocity) / (2 * this.config.deceleration);
					...
					const preventSpinning =
						!this.isShiftPressed || tapRotation || this.activePointers.size > 2;
					...
					if (manualRotation) {
						...
						if (Math.abs(this.currentSpinVelocity) > spinAcceleration * deltaTime) {
							this.currentSpinVelocity -=
								Math.sign(this.currentSpinVelocity) * spinAcceleration * deltaTime;
						} else {
							this.currentSpinVelocity = 0;
						}

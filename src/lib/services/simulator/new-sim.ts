...
export class Simulator extends WritableBase<RobotState> {
    ...
    private readonly targetFacingCircle = document.createElementNS("http://www.w3.org/2000/svg", "circle");
    ...
    start(debug: boolean) {
        ...
        const onmousemove = (e: MouseEvent) => {
            ...
            if (doubleClickTime && (performance.now() - doubleClickTime) < DOUBLE_CLICK_DRAG_THRESHOLD && e.buttons === 1) {
        ...
        const updatePose = () => {
            ...
            if (targetState.position) {
                if (currentState.pose) {
                    ...
                    const hasNewTarget = this.data.isInitialClick && this.data.prevTargetPosition && (
                    ...
                    if (distanceFeet > 0.01) { // Only move if not at target (with small tolerance)
                        const [currentVX, currentVY] = currentState.velocity;
                        const currentSpeed = Math.hypot(currentVX, currentVY);
                        const currentDirection = currentSpeed > 0.01 ? Math.atan2(currentVY, currentVX) : targetDirection;
                        ...
                        if (hasNewTarget && currentSpeed > 0.1) {
                            ...
                            if (Math.abs(normalizedDiff) > Math.PI / 4) { // > 45 degree change
                                // Decelerate first
                            ...
                            const actualSpeedChange = Math.sign(speedDiff) * Math.min(Math.abs(speedDiff), maxSpeedChange);
                    ...
                    if (targetState.orientation !== undefined && currentState.pose) {
                        ...
                        if (Math.abs(angleDiff) > 0.01) { // Only rotate if not at target angle
                            const currentAngularSpeed = Math.abs(currentState.angularVelocity);
                            
                            // Calculate deceleration angle needed to stop
                            const decelAngle = (currentAngularSpeed * currentAngularSpeed) / (2 * angularDeceleration);
                            ...
                            const maxAngularSpeedChange = (angularSpeedDiff > 0 ? angularAcceleration : angularDeceleration) * dt;
                            const actualAngularSpeedChange = Math.sign(angularSpeedDiff) * Math.min(Math.abs(angularSpeedDiff), maxAngularSpeedChange);
    ...
    draw(config: {
        ...
        const pathToDraw = (config.allPoints ? this.points : this.animatedPoints).map(p => (`${p.position[0]},${p.position[1]}`)).join(' ');
        ...
        if (robotPose) {
            ...
            this.robotGroup.setAttribute('transform', `translate(${robotX}, ${robotY}) rotate(${robotAngleDeg})`);
    ...
    //     slowDown?: boolean;
    // }) {
    //     if (!this.running) throw new Error("Simulator is not running");
    ...
    //                 orientation: pose.orientation !== undefined ? (pose.orientation * (Math.PI / 180)) : undefined,
    //             },
    //             positionTolerance: pose.positionTolerance,
    //             orientationTolerance: pose.orientationTolerance,
    //             slowDown: pose.slowDown
    //         });
    ...
    //             targetFacing: undefined,
    //             faceTarget: false,
    //             targetPosition: pose.position ?? state.targetPosition,
    //             targetOrientation: pose.orientation !== undefined ? (pose.orientation * (Math.PI / 180)) : state.targetOrientation,
    //         }))
    //     });
    // }
